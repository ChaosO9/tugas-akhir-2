...
// const JOB_CHANNEL = "job-channel"; // Pub/Sub channel, no longer primary
const JOB_QUEUE_KEY = process.env.JOB_QUEUE_KEY || "job-queue"; // Key for the Redis List
...
const handleRedisMessage = async (
    ...
    // If using BRPOP, 'channel' will be the list key.
    if (channel === JOB_QUEUE_KEY) {
        try {
...
// --- Function to listen for jobs on Redis List using BRPOP ---
async function listenForJobsFromList() {
    console.log(
        `[List Consumer] Listening for jobs on Redis list "${JOB_QUEUE_KEY}"...`
    );
    while (redisSubscriber.isOpen) {
        // Loop as long as the client is open
        try {
            // Block for 0 seconds (indefinitely) until a message is available or client closes
            const result = await redisSubscriber.brPop(JOB_QUEUE_KEY, 0);

            if (result) {
                const { key: listName, element: messageString } = result;
                // Call the existing message handling logic
                // Pass the listName as 'channel' for consistency if handleRedisMessage uses it,
                // or adapt handleRedisMessage if the channel concept is no longer needed.
                console.log(
                    `[List Consumer] Popped message from list "${listName}"`
                );
                await handleRedisMessage(messageString, listName);
            }
            // If result is null, it means brPop was unblocked due to client closing.
            // The loop condition redisSubscriber.isOpen will handle graceful exit.
        } catch (err) {
            if (!redisSubscriber.isOpen) {
                console.log(
                    "[List Consumer] Redis client closed, stopping job listener."
                );
                break;
            }
            console.error(
                `[List Consumer] Error during brPop from list "${JOB_QUEUE_KEY}":`,
                err
            );
            // Implement a short delay before retrying to prevent rapid error loops on other errors
            await new Promise((resolve) => setTimeout(resolve, 5000)); // 5-second delay
        }
    }
    console.log("[List Consumer] Stopped listening for jobs on Redis list.");
}
...
queue.process(async (job: Job<JobData>, done: DoneCallback<any>) => {
    ...
    try {
        ...
        let determinedLogStatus: string = LogStatus.SENT_FAILED;
        let preventRetry = false; // Flag to prevent Bee-Queue retry for this job

        if (axios.isAxiosError(error)) {
            errorMessage = `API request failed: ${error.message}`;
            if (
                error.response?.data?.resourceType === "OperationOutcome" &&
                Array.isArray(error.response.data.issue) // Ensure 'issue' is an array
            ) {
            ...
            if (error.response) {
                ...
                errorMessage += ` - Status ${statusCode}: ${JSON.stringify(
                    errorResponseData
                )}`;
                console.error(`  Status: ${statusCode}`);
                console.error(`  Data: ${JSON.stringify(errorResponseData)}`);

                // If not already marked as non-retriable by OperationOutcome, check status code
                if (!preventRetry) {
                    if (
                        statusCode >= 400 &&
                        statusCode < 500 &&
                        ...
                        // Other 4xx errors (excluding 429) are generally non-retriable
                        preventRetry = true;
                        // determinedLogStatus remains SENT_FAILED unless changed by OperationOutcome
                    }
                    // For 429 (Too Many Requests), 5xx, or other errors, preventRetry remains false (i.e., allow retry)
                }
            ...
            // This error implies a problem before making the API call, potentially transient.
            errorMessage = error.message;
            determinedLogStatus = LogStatus.ERROR; // Token error is not an API send failure, but should be retried.
            console.error(`  Authentication failed.`);
        } else {
            errorMessage =
                error instanceof Error ? error.message : String(error);
            // Other unexpected errors, potentially transient, should be retried.
        ...
        if (preventRetry) {
            console.log(
                `[${job_uuid}] Job ${
                    job.id
                } failed with a non-retriable client error (HTTP Status: ${
                    error.response?.status || "N/A"
                }, Determined Status: ${determinedLogStatus}). Preventing further retries.`
            );
            job.retries(0); // Tell Bee-Queue not to retry this job anymore.
            // This modifies the job object in memory, which Bee-Queue uses for its retry decision.
        }
...
async function startService() {
    try {
        ...
        // Start listening to the Redis List instead of Pub/Sub
        listenForJobsFromList().catch((err) => {
            console.error(
                "[List Consumer] Job listener loop encountered an unrecoverable error:",
                err
            );
            shutdown(1); // Trigger shutdown if the listener loop crashes
        });

        // If you still need to listen to the old Pub/Sub channel for a transition period, you can keep this:
        // await redisSubscriber.subscribe(JOB_CHANNEL, handleRedisMessage);
        // console.log(`Subscribed to "${JOB_CHANNEL}" on Redis Pub/Sub for legacy messages.`);
        console.log(
...
async function shutdown(forcedExitCode?: number) {
    ...
    if (redisSubscriber.isOpen) {
        try {
            // No explicit unsubscribe needed for BRPOP.
            // If you were still subscribed to JOB_CHANNEL for transition, then:
            // if (await redisSubscriber.isSubscribed(JOB_CHANNEL)) { // Check if actually subscribed
            //     await redisSubscriber.unsubscribe(JOB_CHANNEL);
            //     console.log("Unsubscribed from Redis Pub/Sub channel.");
            // }
